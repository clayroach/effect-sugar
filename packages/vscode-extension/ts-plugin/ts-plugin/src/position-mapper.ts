...
export class PositionMapper {
  ...
  originalToTransformed(pos: number): number {
    // Bounds check
    if (pos < 0) return 0
    if (pos >= this.originalSource.length) return this.transformedSource.length
    ...
    if (generated.line === null || generated.column === null) {
      ...
      return Math.min(pos, this.transformedSource.length)
    }

    const result = this.lineColumnToPosition(this.transformedSource, generated.line, generated.column)
    return Math.max(0, Math.min(result, this.transformedSource.length))
  }
  ...
  transformedToOriginal(pos: number): number {
    // Bounds check
    if (pos < 0) return 0
    if (pos >= this.transformedSource.length) return this.originalSource.length
    ...
    if (original.line === null || original.column === null) {
      // Fallback: return position as-is if mapping fails
      return Math.min(pos, this.originalSource.length)
    }

    const result = this.lineColumnToPosition(this.originalSource, original.line, original.column)
    return Math.max(0, Math.min(result, this.originalSource.length))
  }

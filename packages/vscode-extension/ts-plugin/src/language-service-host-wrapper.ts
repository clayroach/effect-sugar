...
import {
  ...
  getOriginalSource as getOriginalSourceFromCache,
  getTransformedSource as getTransformedSourceFromCache,
  type PositionMapper,
  type SourceMapData
} from './position-mapper.js'
import { hasGenBlocks, transformSource } from './transformer.js'
...
export interface WrappedHostResult {
  ...
  /** Get original source for a file */
  getOriginalSource: (fileName: string) => string | undefined
  /** Get transformed source for a file */
  getTransformedSource: (fileName: string) => string | undefined
}
...
export function createWrappedLanguageServiceHost(
  ...
  return {
    ...
    getOriginalSource(fileName: string): string | undefined {
      return getOriginalSourceFromCache(fileName)
    },

    getTransformedSource(fileName: string): string | undefined {
      return getTransformedSourceFromCache(fileName)
    }
  }
}

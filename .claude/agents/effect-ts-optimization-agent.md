...
- **Example Fix**:
  ...
  // NOTE: Effect.gen is appropriate here for service construction with dependencies.
  // See Section 9 "Effect.gen Decision Tree" for when to use generators vs simpler alternatives.
  ```
...
### 9. Effect.gen Decision Tree: When to Use Generators

**Effect.gen is a powerful tool - use it appropriately, not everywhere.**

#### ALWAYS Use Effect.gen For:

**1. Service Construction with Dependencies (see Section 4.5)**
```typescript
// ✅ CORRECT: Service construction needs Effect.gen for dependency injection
const ServiceLayer = Layer.effect(
  ServiceTag,
  Effect.gen(function* (_) {
    const dep1 = yield* _(Dependency1Tag)
...
```typescript
// ✅ CORRECT: Conditionals, loops, try-catch benefit from generator syntax
Effect.gen(function* (_) {
...
```typescript
// ✅ CORRECT: Later operations depend on earlier results
Effect.gen(function* (_) {
  const config = yield* _(loadConfig())
  const client = yield* _(createClient(config.endpoint))
  const session = yield* _(authenticate(client, config.credentials))
  return { client, session }
})
```

#### NEVER Use Effect.gen For:

**1. Single Operations** → Use direct pipe
```typescript
// ❌ BAD
Effect.gen(function* (_) {
...
**2. Static Values** → Use Effect.succeed
```typescript
// ❌ BAD
Effect.gen(function* () {
  return { value: 42 }
...
**3. Simple Transformations** → Use Effect.map
```typescript
// ❌ BAD
Effect.gen(function* (_) {
  const config = yield* _(loadConfig())
...
// ✅ GOOD
loadConfig().pipe(
  Effect.map(config => ({ ...config, enabled: true }))
)
```

**4. Simple Sequences** → Use Effect.flatMap
```typescript
// ❌ BAD
Effect.gen(function* (_) {
  const a = yield* _(effectA)
  return yield* _(effectB(a))
})
...
**5. Parallel Operations** → Use Effect.all
```typescript
// ❌ BAD
Effect.gen(function* (_) {
...
```
Does this code construct a service with dependencies?
  └─ YES → Use Effect.gen (Section 4.5 requirement)
  └─ NO → Continue...

Does this code have complex control flow (if/else, loops, try-catch)?
  └─ YES → Use Effect.gen (improves readability)
  └─ NO → Continue...

Does this code have 3+ sequential operations with interdependencies?
  └─ YES → Use Effect.gen (generator syntax clearer)
  └─ NO → Continue...

Is this a single operation, simple transformation, or parallel operations?
  └─ YES → DO NOT use Effect.gen (use pipe, map, flatMap, or all)
  └─ NO → Evaluate case-by-case (prefer simpler alternatives)
```

### 10. Effect.gen Anti-Pattern Detection: When to Avoid Generators
**NOTE: This section focuses on SIMPLE operations that don't need generators. For valid Effect.gen use cases (service construction, complex control flow), see Section 9 above.**

**Unnecessary Effect.gen usage for simple operations is a code smell indicating over-engineering:**
...
**NOTE: These anti-patterns apply to simple operations only. They do NOT apply to:**
- Service construction with dependencies (Section 4.5)
- Complex control flow (if/else, loops, try-catch)
- Multiple interdependent operations (3+ yields with dependencies)

See Section 9 "Effect.gen Decision Tree" for when Effect.gen IS appropriate.
